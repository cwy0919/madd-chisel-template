...
class MarkovPrefetcher extends Module {
  ...
  switch(stateReg) {
    is(sIdle) {
      ...
      // previousAddressReg := 0.U
      // previousAddressValidReg := 0.U
      hitReg := false.B
      hitIndexReg := 0.U
      prefetchHitReg := false.B
      demandHitReg := false.B
      prefetchReg := false.B
      prefetchAddressReg := 0.U
      mostProbableNextAddressReg := 0.U
      mostProbableNextAddressValidReg := false.B
      mostProbableNextAddressInHistoryReg := false.B
    ...
    is(sFindHit) {
      ...
      hitReg := hit
      hitIndexReg := hitIndex
      prefetchHitReg := prefetchHit
      demandHitReg := demandHit
      // TODO: 更新相关的寄存器
    ...
    is(sUpdateHistory1) {
      when(hitReg) {
        when (prefetchHitReg) {
          accessHistoryReg(hitIndexReg).prefetch := false.B
        }
          accessHistoryReg(hitIndexReg).timestamp := cycleCounterReg
        // TODO: 更新命中条目的时间戳
        }.otherwise {
          val oldestEntryIndex = MarkovPrefetcherUtils.findOldestEntryIndex(accessHistoryReg)
          accessHistoryReg(oldestEntryIndex).address := currentAddressReg
          accessHistoryReg(oldestEntryIndex).valid := true.B
          accessHistoryReg(oldestEntryIndex).prefetch := false.B
          accessHistoryReg(oldestEntryIndex).timestamp := cycleCounterReg
        // TODO: 替换历史中最老的条目
      }
    ...
    is(sFindMostProbable) {
      ...
      prefetchReg :=mostProbableNextAddressValid && !mostProbableNextAddressInHistory
      prefetchAddressReg := mostProbableNextAddress
      mostProbableNextAddressReg:= mostProbableNextAddress
      mostProbableNextAddressValidReg := mostProbableNextAddressValid
      mostProbableNextAddressInHistoryReg := mostProbableNextAddressInHistory
      // TODO: 更新相关的寄存器
    ...
    is(sUpdateHistory2) {
      when(previousAddressValidReg) {
        transitionTableReg(previousAddressReg)(currentAddressReg) := transitionTableReg(previousAddressReg)(currentAddressReg) + 1.U

        // TODO: 更新状态转移表
      }
      when(prefetchReg) {
        val oldestIndex = MarkovPrefetcherUtils.findOldestEntryIndex(accessHistoryReg)
        accessHistoryReg(oldestIndex).address := prefetchAddressReg
        accessHistoryReg(oldestIndex).valid := true.B
        accessHistoryReg(oldestIndex).prefetch := true.B
        accessHistoryReg(oldestIndex).timestamp := cycleCounterReg

        // TODO: 用预取地址替换最老的条目
      }
        previousAddressReg := currentAddressReg
        previousAddressValidReg := true.B
